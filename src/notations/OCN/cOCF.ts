...
const cOCF_count = (x: any): any => (x.match(/\(/g) || []).length - (x.match(/\)/g) || []).length;

function cOCF_op(x: any): any {
    if (cOCF_lt(x, 'p(p(0))')) {
        return false;
    }
...
function cOCF_display(x: any): any {
    //console.log(x);
    if (x + '' == 'Infinity') {
        return 'c';
    }
...
function cOCF_paren(x: any, n: any): any {
    console.log();
    let q = x[n] == '(' ? 1 : -1;
    let i = n;
    let t = 0;
...
function cOCF_arg(x: any): any {
    console.log();
    return cOCF_firstTerm(x)[0].slice(2, -1);
}

function cOCF_lt(x: any, y: any): any {
    console.log();
    if (y == '0') {
        return false;
    }
...
function cOCF_lv(x: any): any {
    return cOCF_expW(cOCF_lastTerm(cOCF_arg(x)).at(-1));
}

function cOCF_fix(s: any): any {
    while (cOCF_count(s)) {
        s += ')';
    }
    return s;
...
function cOCF_fs(x: any, n: any): any {
    if (x == '0') {
        return x;
    }
    let y = x;
...
function cOCF_add(x: any, y: any): any {
    if (x == '0') {
        return y;
    }
    if (y == '0') {
...
function cOCF_sub(x: any, y: any): any {
    if (x == '0') {
        return '0';
    }
    if (y == '0') {
...
function cOCF_sua(x: any): any {
    return cOCF_split(x, 'P(0)');
}

function cOCF_exp(a: any): any {
    if (a[0] == 'P') {
...
function cOCF_div(a: any, b: any): any {
...
function cOCF_mul(a: any, b: any): any {
...
function cOCF_split(a: any, x: any): any {
    if (a == '0') {
        return ['0', '0'];
    }
...
function cOCF_findall(a: any): any {
    if (a == '0') {
        return [];
    }
    ...
    return cOCF_terms(p)
        ...
        .flat()
        .filter((x: any) => x != '0')
        .concat([q].filter((x: any) => x != '0'));
}

function cOCF_compare(x: any, y: any): any {
    if (cOCF_lt(x, y)) {
        return -1;
    }
    if (cOCF_lt(y, x)) {
...
function cOCF_islimit(a: any): any {
    if (a + '' == 'Infinity') {
        return true;
    }
    if (a == '0') {
...
function cOCF_display_plain(x: any): any {
    return cOCF_display(x)
        .replace(/<sup>/g, '^(')
        .replace(/<\/sup>/g, ')')
        .replace(/<sub>/g, '_{')
        .replace(/<\/sub>/g, '}')
...
export const cOCF: NotationDefinition<any> = {
    id: 'cocf',
    name: 'cOCF',

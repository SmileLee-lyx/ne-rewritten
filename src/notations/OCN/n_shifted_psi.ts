...
const HSPN_count = (x: any): any => (x.match(/\(/g) || []).length - (x.match(/\)/g) || []).length;

function unabbreviate(x: any): any {
    let y = x;
    ...
    y = y.replaceAll('_', '');
    y = y.replaceAll(/W\d+/g, (p: any) => 'W'.repeat(Number(p.slice(1))));

    function e(x: any) {
        return x.replaceAll(/p\(W{2,}\)/g, (p: any) => 'W'.repeat(p.length - 4));
    }
    ...
    y = y.replaceAll(/[1-9]\d*/g, (p: any) => {
        return 'p(0)+'.repeat(Number(p)).slice(0, -1);
    });
    return y;
}

function abbreviate(x: any): any {
    let y = x;
    y = y.replaceAll(/W{2,}/g, (p: any) => `W_${p.length}`);
    y = y.replaceAll('W', 'Ω');
    y = y.replaceAll('p(0)', '1');
    y = y.replaceAll(/(1\+)+1/g, (p: any) => ((p.length + 1) / 2).toString());
    y = y.replaceAll('p(1)', 'ω');
    y = y.replaceAll('p', 'ψ');
    return y;
}

function HSPN_std(x: any): any {
    return x == '' ? 0 : unabbreviate(abbreviate(x));
}

function HSPN_paren(x: any, n: any, sw = true): any {
    if (x[n - 1] == 'W' && sw) {
        n--;
        let i = n;
        while (x[i] == 'W') {
...
function HSPN_lt(x: any, y: any): any {
    if (y == '0') {
        return false;
    }
    if (x == '0') {
...
function HSPN_limit(s: any, n: any): any {
    return 'p('.repeat(n + 1) + 'W'.repeat(s + n) + '+' + 'W'.repeat(s + n) + ')'.repeat(n + 1);
}

function HSPN_fix(s: any): any {
    while (HSPN_count(s)) {
        s += ')';
    }
    return s;
...
function HSPN_islimit(x: any): any {
    if ('' + x == 'Infinity') {
        return true;
    }
    if (x == '0') {
...
function HSPN_root(x: any, l: any): any {
    let b = x.length - 1;
    while (x[b] == ')') {
        b--;
    }
...
function HSPN_fs(x: any, n: any): any {
    if (x == '0') {
        return x;
    }
    if (x.at(-1) == 'W') {
...
function HSPN_display(x: any): any {
    if ('' + x == 'Infinity') {
        return 'Limit';
    }
    return abbreviate(x).replaceAll(/_\d+/g, (x: any) => `<sub>${Number(x.slice(1))}</sub>`);
}

function HSPN_compare(x: any, y: any): any {
    if (HSPN_lt(x, y)) {
        return -1;
    }
    if (HSPN_lt(y, x)) {
...
function HSPN_display_plain(x: any): any {
    return HSPN_display(x)
        .replace(/<sub>/g, '_')
        .replace(/<\/sub>/g, '');
}

export const n_shifted_psi: NotationDefinition<any> = {
    id: 'hspn',
    name: 'n-shifted psi',
    simple_name: 'n-dp',

...
function compute_up_2mn(expr: Expr, P: Position[][], [Ri, Rj]: Position): boolean[] {
    ...
    for (let i = Ri + 1; i < expr.length; i++) {
        ...
        do {
            const X_start = i;
            let Y_start = right;
            while (expr[Y_start].length !== Rj + 2 || P[Y_start][Rj][0] !== Ri) {
...
export const draw_diagram_control: DiagramControl<Expr, DiagramData> = {
    ...
    draw_diagram: (_expr, _data) =>
        draw_up2dbms_v1c_mountain_diagram(_expr, _data.current_equiv, _data.invert_vertical ?? false),
...
export const UP2DBMS_v1c: NotationDefinition<Expr> = {
    id: 'up2dbms-v1c',
    name: 'UP2DBMS v1C',
    description: [
        { id: 'description.up2dbms-v1c.1' },
        { id: 'description.up2dbms-v1c.2' },
        { id: 'description.up2dbms-v1c.3' },
    ],

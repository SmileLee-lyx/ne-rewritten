...
function build_up2dbms_v1_mountain_source(m: Expr, current_equiv: string | undefined): MountainViewSource | undefined {
    ...
    const shape: MountainShape<Vertical> = m.map((col, i) => {
        const nodes: MountainNode<Vertical>[] = [{ vertical: [], text: is_y ? '' + m_1y[i][0] : '*' }];
...
function draw_up2dbms_v1_mountain_diagram(
    m: Expr,
    current_equiv: string | undefined,
    invert_vertical: boolean,
): Diagram | undefined {
    const source = build_up2dbms_v1_mountain_source(m, current_equiv);
...
export const draw_diagram_control: DiagramControl<Expr, DiagramData> = {
    ...
    draw_diagram: (_expr, _data) =>
        draw_up2dbms_v1_mountain_diagram(_expr, _data.current_equiv, _data.invert_vertical ?? false),
...
export const UP2DBMS_v1: NotationDefinition<Expr> = {
    ...
    mountain_view: (expr, data) => build_up2dbms_v1_mountain_source(expr, data?.current_equiv),

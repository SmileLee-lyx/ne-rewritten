/**
 * strong e0MN(strong e0 Mountain Notation)记号定义。
 *
 * 由 `other-notation/strong_e0MN.js`(原作者 test_alpha0 的 NER 自定义记号脚本)改写而来:
 * 函数命名与算法逻辑与原文件严格一致, 仅补充类型标注;
 * 唯一的行为性改动是按项目风格把极限哨兵由字符串 `'Limit'` 改为 `Infinity as any`,
 * 并把原 `makeLimit` 换成本项目的 `infinity_FS`。
 *
 * 与 `e0MN.ts`(非 strong 版)的关系: 两者是**各自独立的副本**(不共享模块、不建立依赖),
 * 且展开器算法按各自的原脚本重写, 只有三处实质差异:
 *   1. `shiftOrdNumbers`: strong 版把指数项也递归移位, 且判据为 `coeff >= cutoff`
 *      (含省略的 1, 如 ω = ω^1·1 在 cutoff=1,d=2 下变成 ω^3·3);
 *   2. `down` 的 `omit` 判据: `ordEq(xnL, last[n-2].x)`(相等)而非 `ordCompare(...) <= 0`(不大于);
 *   3. `shortExpansion` 的 `omit` 判据: `ordEq(xnL, prev)` 而非 `ordCompare(xnL, prev) <= 0`。
 * 记号 id / name / simple_name 亦不同。
 */
...
// Strong e0MN: every positive integer occurring in a row-label e0 shifts iff q >= cutoff.
// This includes omitted 1s: e.g. ω = ω^1·1, so with cutoff=1,d=2 it becomes ω^3 3.
function shiftOrdNumbers(a: Ord, cutoff: number, d: number): Ord {
    return a.map((t) => ({
        exp: shiftOrdNumbers(t.exp, cutoff, d),
        coeff: t.coeff >= cutoff ? t.coeff + d : t.coeff,
    }));
}
...
function shortExpansion(src: Expr): Expr {
    ...
    const omit = isOne(xn) || (n > 1 && ordEq(xnL, prev));
...
export const strong_e0MN: NotationDefinition<Expr> = {
    id: 'strong-e0mn',
    name: 'strong e0 Mountain Notation',
    simple_name: 'strong e0MN',

...
type HeightPos = [number, number | undefined];
...
function compare_height_pos(hp1: HeightPos, hp2: HeightPos): number {
    return lex_compare(hp1, hp2, compare_undefined_last_by(number_compare));
}

function compare_height(h1: Height, h2: Height): number {
    return lex_compare(h1, h2, tuple_lex_compare_by([number_compare, compare_height_pos]), false);
}
...
function height_pos_display(p: HeightPos): string {
    if (p[1] === undefined) return 'ω' + (p[0] === 0 ? '' : '' + (p[0] + 1));
    if (p[0] === 0) return '' + (p[1] + 1);
    return 'ω' + (p[0] === 1 ? '' : '' + p[0]) + '+' + (p[1] + 1);
}
...
export function from_display(str: string): Expr {
    ...
    /** 行高的值: 数字 n 的内部值 = n - 1。行高的值没有 ω 等特殊值(特殊值只在行高的位置上)。 */
    function parse_height_value(): number {
        return parse_number() - 1;
    }

    /**
     * 行高的位置, 与 height_pos_display 互逆:
     * 'ω'                       → [0, undefined];
     * 'ω' + (n+1)               → [n, undefined](n >= 1);
     * '(m+1)'                 → [0, m];
     * 'ω' + (n===1 ? '' : n) + '+' + (m+1) → [n, m](如 'ω2+6' → [2, 5]、'ω+6' → [1, 5])。
     */
    function parse_height_pos(): HeightPos {
        ...
        if (i < s.length && (s[i] === 'ω' || s[i] === 'w')) {
            i++;
            skip_spaces();
            const n = i < s.length && s[i] >= '0' && s[i] <= '9' ? parse_number() : 1;
            skip_spaces();
            if (i < s.length && s[i] === '+') {
                i++;
                return [n, parse_number() - 1];
            }
            if (n === 1) return [0, undefined];
            return [n - 1, undefined];
        }
        return [0, parse_height_value()];
    }
...
function copy_value<T extends number | undefined>(value: T, r: number, offset: number): T;
...
function copy_height_pos([p1, p0]: HeightPos, r: number, offset: number): HeightPos {
    return [p1, copy_value(p0, r, offset)];
}
...
function find_lower_height_pos(expr: Expr, h: Height, p: HeightPos, r: number): HeightPos | undefined {
    if (p[1] === undefined) return [p[0], r];

    const default_value: HeightPos | undefined = p[0] === 0 ? undefined : [p[0] - 1, undefined];

    const bound = find_maximal_entry_below(expr, p[1], h, r);
    if (bound === undefined) return default_value;
...
type Sep_DBMS = [number, number];
type Entry_DBMS = [number, Sep_DBMS];
type Column_DBMS = Entry_DBMS[];
type Expr_DBMS = Column_DBMS[];

function layered_top_separator(expr: Expr, h: Height, r: number): Sep_DBMS {
    const [, p] = h[h.length - 1];

    let layer: Sep_DBMS = [0, 0];
    let current: HeightPos | undefined = p;
    while (true) {
...
function dbms_sep_display(s: Sep_DBMS): string {
    if (s[0] === 0) return ','.repeat(s[1] + 1);
    return ';'.repeat(s[0]) + ','.repeat(s[1]);
}
...
type Vertical_DBMS = Sep_DBMS[];

function compare_dbms_sep(s1: Sep_DBMS, s2: Sep_DBMS): number {
    return lex_compare(s1, s2, number_compare);
}

function compare_dbms_vertical(v1: Vertical_DBMS, v2: Vertical_DBMS): number {
    return lex_compare(v1, v2, compare_dbms_sep);
}

function dbms_vertical_increase(v: Vertical_DBMS, s: Sep_DBMS): Vertical_DBMS {
...
function display_as_Y(matrix: Expr_DBMS): string {
    if (is_infinity_dbms(matrix)) return '1,3,15';
...
export const S_omega_square_DBMS: NotationDefinition<Expr> = {
    id: 's-omega^2-dbms',
    name: 'S ω^2 DBMS',
    ...
    description: [{ id: 'description.s-omega2-dbms.1' }],

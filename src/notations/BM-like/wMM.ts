...
const vertical_compare = (a: any, b: any): any => {
...
const vertical_increase = (y: any, d: any): any => {
    var c = y.slice();
...
const extract = (A: any, [x, y]: any): any => A[x][y] || 0;

const get_vertical = (A: any, [x, y]: any): any => {
    var val: any;
    if (vertical_cache.has(A)) {
        val = vertical_cache.get(A);
    } else {
        val = A.map((column: any, x: any) => {
            var result: any = [],
                i: any,
                y: any = 0;
...
const parentCheck = (A: any, [x, y]: any): any => {
    if (!y) return [x - 1, y];
...
const parent = (A: any, cur: any): any => {
...
const expand = (M: any, FSterm: any): any => {
    var LNZx = M.length - 1;
    var LNZy = M[LNZx].findLastIndex((e: any) => e);
    var LNZ = M[LNZx][LNZy];
    var collection: any = [];
    ...
    var counts = collection.filter(() => true).map((e: any) => e.length);
    var columns = collection.map((e: any, i: any) => i).filter(() => true);
    counts.unshift(1);
    ...
    var A = M.map((column: any) => column.slice());

    --A[LNZx][LNZy];

    M[root[0]].slice(root[1]).forEach((val: any, dy: any) => {
        A[LNZx][LNZy + dy] = val;
    });

    var ascending_cache: any = {};

    const ascendingAt = (cur: any): any => {
        var str = '' + cur;
    ...
    for (var n = 1; n <= FSterm; ++n) {
        ...
        for (var dx = 1; dx <= width; ++dx) {
            var x = root[0] + dx;
            var targetColumn: any = (A[x + width * n] = []);
            var lastmagma = -1;

            A[x].forEach((val: any, y: any) => {
    ...
    A.forEach((column: any) => {
        var i = column.findLastIndex((e: any) => e);
        column.splice(i + 1);
    });
